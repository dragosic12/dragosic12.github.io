...
export function ImageLabSection({ locale, imageLab }: ImageLabSectionProps) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setError(
        locale === 'es'
          ? 'No se pudo generar la imagen con los proveedores disponibles. Revisa la API del servidor o vuelve a intentarlo.'
          : 'Could not generate the image with available providers. Check server API status or try again.'
      );
    } finally {
      setIsLoading(false);
    }
